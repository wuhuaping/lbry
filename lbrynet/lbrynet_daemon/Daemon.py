...
class Daemon(AuthJSONRPCServer):
    ...
    def _find_lbry_file_by_uri(self, name):
        for lbry_file in self.lbry_file_manager.lbry_files:
            if name == lbry_file.name:
                return lbry_file
    ...
    def _find_lbry_file_by_claim_id(self, claim_id):
        for lbry_file in self.lbry_file_manager.lbry_files:
            if lbry_file.claim_id == claim_id:
                return lbry_file
        raise Exception("File for claim %s not found" % claim_id)

    def _find_lbry_file_by_outpoint(self, outpoint):
        for lbry_file in self.lbry_file_manager.lbry_files:
            if repr(lbry_file.outpoint) == outpoint:
                return lbry_file
        raise Exception("File for outpoint %s not found" % outpoint)
    ...
    @defer.inlineCallbacks
    def _get_lbry_file_by_claim_id(self, claim_id):
        lbry_file = yield self._find_lbry_file_by_claim_id(claim_id)
        defer.returnValue(lbry_file)

    @defer.inlineCallbacks
    def _get_lbry_file_by_outpoint(self, outpoint):
        lbry_file = yield self._find_lbry_file_by_outpoint(outpoint)
        defer.returnValue(lbry_file)
    ...
    def _get_lbry_file_dict(self, lbry_file):
        ...
        if lbry_file.sd_hash:
            metadata = yield self.storage.get_claim_metadata_for_sd_hash(lbry_file.sd_hash)
        else:
            metadata = {}
        defer.returnValue({
    ...
    def _get_lbry_file(self, search_by, val, return_json=True):
        if search_by == FileID.CLAIM_ID:
            lbry_file = yield self._get_lbry_file_by_claim_id(val)
        elif search_by == FileID.FILE_NAME:
            lbry_file = yield self._get_lbry_file_by_file_name(val)
        elif search_by == FileID.SD_HASH:
            lbry_file = yield self._get_lbry_file_by_sd_hash(val)
        elif search_by == FileID.STREAM_HASH:
            lbry_file = yield self._get_lbry_file_by_stream_hash(val)
        elif search_by == FileID.NAME:
            lbry_file = yield self._get_lbry_file_by_uri(val)
        elif search_by == FileID.OUTPOINT:
            lbry_file = yield self._get_lbry_file_by_outpoint(val)
        else:
    ...
    @defer.inlineCallbacks
    def _get_lbry_files(self, as_json=False, **kwargs):
        lbry_files = list(self.lbry_file_manager.lbry_files)
        if kwargs:
            for search_type, value in kwargs.iteritems():
                lbry_file = yield self._get_lbry_file(search_type, value, return_json=False)
                lbry_files = [l_f for l_f in lbry_files if
                              l_f.__dict__[search_type] == lbry_file.__dict__[search_type]]
        if as_json:
            file_dicts = []
    ...
    def jsonrpc_file_list(self, **kwargs):
        """
        List files limited by option filters
