...
class AuthJSONRPCServer(AuthorizedBase):
    ...
    def _render_error(self, failure, request, id_, version=jsonrpclib.VERSION_2, response_code=FAILURE):
        ...
        if self._use_authentication:
            if function_name in self.authorized_functions:
                try:
                    ...
                    self._render_error(
                        err, request, id_, version=version, response_code=AuthJSONRPCServer.UNAUTHORIZED)
                    return server.NOT_DONE_YET
