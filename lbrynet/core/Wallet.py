...
class WalletNotRunning(Exception):
    def __init__(self, f, *args):
        self.message = "Wallet is stopped, cannot run %s(%s)" % (f, args)


class ThreadedCallManager(object):
    """
...
class Wallet(ThreadedCallManager):
    ...
    def initialize_claims(self):
        claims_missing_metadata = yield self._storage.repair_claims()
        if claims_missing_metadata:
            log.info("Fixing %i claims missing metadata", len(claims_missing_metadata))
            for outpoint in claims_missing_metadata:
        ...
        log.info("Checking %i claims", len(my_claims))
    ...
    def _get_stream_info_from_value(self, result, name):
        ...
        d = self.get_claimid(name, result['txid'], result['n'])
    ...
    def _get_claim_info(self, name, claim_outpoint):
        ...
        yield self._save_name_metadata(name, claim_outpoint, claim['claimId'],
