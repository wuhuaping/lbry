...
class MemoryStorage(object):
    ...
    def repair_claims(self):
        to_repair = yield self.query("SELECT claim_id, sd_blob_id FROM files")
        for claim_row, sd_row in to_repair:
            yield self.query("UPDATE claims SET sd_blob_id=? WHERE id=?", (sd_row, claim_row))
        results = yield self.query("SELECT txid, nout FROM claims WHERE status=?", ("MISSING_METADATA", ))
